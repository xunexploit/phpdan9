	...
	<div class="clearfix">
		<div class="search-box we7-margin-bottom">
			<select class="we7-margin-right">
				<option data-url="<?php  echo filter_url('module_type:');?>" <?php  if(empty($_GPC['module_type'])) { ?>selected<?php  } ?>>模块类型</option>
				<option data-url="<?php  echo filter_url('module_type:1');?>" <?php  if($_GPC['module_type'] == STORE_TYPE_MODULE) { ?>selected<?php  } ?>>公众号应用</option>
				<option data-url="<?php  echo filter_url('module_type:4');?>" <?php  if($_GPC['module_type'] == STORE_TYPE_WXAPP_MODULE) { ?>selected<?php  } ?>>小程序应用</option>
			</select>
	...
	<div class="store module">
		...
		<?php  if(is_array($store_goods)) { foreach($store_goods as $goods) { ?>
			...
			<a href="<?php  echo $this->createWebUrl('goodsbuyer', array('operate' => 'goods_info', 'direct' => 1, 'goods' => $goods['id']))?>">
				...
				<?php  } else if(in_array($goods['type'], array(STORE_TYPE_PACKAGE, STORE_TYPE_ACCOUNT, STORE_TYPE_WXAPP, STORE_TYPE_ACCOUNT_RENEW, STORE_TYPE_WXAPP_RENEW, STORE_TYPE_ACCOUNT, STORE_TYPE_WXAPP))) { ?>
				...
				<?php  } else if(in_array($goods['type'], array(STORE_TYPE_MODULE, STORE_TYPE_WXAPP_MODULE))) { ?>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr class="submenu">
							<td class="td_one" colspan="3">
								...
								<span class="icon">
									<i class="wi wi-account"></i> <?php  echo $user_groups[$goods['user_group']]['maxaccount'];?>
			...
			<div class="panel we7-panel">
				<div class="panel-body">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr class="group_menu">
							<td class="td_1"><span class="head"><i class="wi wi-userjurisdiction"></i></span></td>
							<td class="td_2"><span class="name"><?php  echo $account_groups[$goods['account_group']]['group_name'];?></span><span class="price">¥ <?php  echo $goods['price'];?>元/<?php  if($goods['unit'] == 'month') { ?>月<?php  } else if($goods['unit'] == 'day') { ?>日<?php  } else if($goods['unit'] == 'year') { ?>年<?php  } ?></span></td>
							<td class="td_3">
						...
						<tr class="submenu">
							<td class="td_one" colspan="3">
								<span class="tit">可创建</span>
								<span class="icon">
									<i class="wi wi-account"></i> <?php  echo $account_groups[$goods['account_group']]['maxaccount'];?>
									<i class="wi wi-wxapp" ></i> <?php  echo $account_groups[$goods['account_group']]['maxwxapp'];?>
									<i class="wi wi-pc-circle"></i> <?php  echo $account_groups[$goods['account_group']]['maxwebapp'];?>
									<i class="wi wi-app"></i> <?php  echo $account_groups[$goods['account_group']]['maxphoneapp'];?>
								</span>
							</td>
						</tr>
					</table>
		...
		<div class="top">
			...
			<div class="buy pull-right">
				<div class="price">单价￥<span class="fee">
					...
					<?php  if($goods['type'] != STORE_TYPE_API && !in_array($goods['type'], array(STORE_TYPE_ACCOUNT, STORE_TYPE_WXAPP))) { ?>/<?php  if($goods['unit'] == 'month') { ?><?php  if($goods['type'] == STORE_TYPE_ACCOUNT_RENEW) { ?><?php  echo $goods['account_num'];?><?php  } else if($goods['type'] == STORE_TYPE_WXAPP_RENEW) { ?><?php  echo $goods['wxapp_num'];?><?php  } ?>月
				...
				<div class="buy-btn">
					<?php  if(in_array($goods['type'], array(STORE_TYPE_MODULE, STORE_TYPE_WXAPP_MODULE, STORE_TYPE_PACKAGE, STORE_TYPE_ACCOUNT_RENEW, STORE_TYPE_WXAPP_RENEW, STORE_TYPE_USER_PACKAGE, STORE_TYPE_ACCOUNT_PACKAGE))) { ?>
						...
						<form action="" method="get" class="we7-form">
							...
							<div class="form-group">
								<label class="control-label col-sm-2">
									<?php  if($goods['type'] == STORE_TYPE_WXAPP_MODULE || $goods['type'] == STORE_TYPE_WXAPP_RENEW) { ?>
									小程序
									<?php  } else if($goods['type'] == STORE_TYPE_MODULE || $goods['type'] == STORE_TYPE_ACCOUNT_RENEW) { ?>
										公众号
									<?php  } ?>
								</label>
								<div class="col-sm-10">
			...
			<div class="creat">
				...
				<ul class="clearfloat">
					<li>
						...
						<div class="num"><?php  echo $group_info['maxaccount'];?></div>
					</li>
					<li>
						<div class="type"><div>小程序</div><span><i class="wi wi-wxapp"></i></span></div>
						<div class="num"><?php  echo $group_info['maxwxapp'];?></div>
					</li>
					<li>
						<div class="type"><div>PC应用</div><i class="wi wi-pc"></i></div>
						<div class="num"><?php  echo $group_info['maxwebapp'];?></div>
					</li>
					<li>
						<div class="type"><div>APP</div><i class="wi wi-app"></i></div>
						<div class="num"><?php  echo $group_info['maxphoneapp'];?></div>
					</li>
				</ul>
			</div>
			<?php  if($goods['type'] == STORE_TYPE_USER_PACKAGE) { ?>
