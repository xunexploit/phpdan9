...
<div class="fui-page cav order_detail">
	...
	<div class="fui-content navbar <?php  echo $seckill_color;?><?php  if($order['refundstate']==3) { ?>singrefund<?php  } ?>">
		...
		<div class="order_detail_header">
			<div class="order_detail_ststus">
				<div style="font-size: 0.85rem;">
					...
					<?php  if($order['refundstate'] > 0 && $order['refundstate']!=3) { ?>
					(申请售后中)
					<?php  } ?>
		...
		<div class="fui-list-group">
			...
			<a class="fui-list" href="<?php  if(empty($order['isnewstore'])) { ?><?php  echo mobileUrl('goods/detail',array('id'=>$g['goodsid']))?><?php  } else { ?><?php  echo mobileUrl('newstore/goods/detail',array('id'=>$g['goodsid'],'storeid'=>$order['storeid']))?><?php  } ?>">
				...
				<div class="fui-list-angle" style="margin-right: 0">
					...
					<?php  if(($g['refund'] || $g['returngoods'] || $g['exchange']) && $is_single_refund) { ?>
						<div class="fui-cell-remark">
							<div class="btn btn-sm btn-default-o" style="height: 22px;line-height: 20px;border-radius: 22px;font-size: 0.6rem;width: 2.7rem;padding: 0;text-align: center;" onclick="window.location.href='<?php  echo mobileUrl('order/single_refund', array('id'=>$g['id']))?>'">
								...
								<?php  } else if($g['single_refundstate']==2) { ?>
									退货中
								<?php  } else if($g['single_refundstate']==0) { ?>
									售后
								<?php  } ?>
							</div>
						</div>
			...
			<a href="<?php  echo mobileUrl('member/fullback',array('orderid'=>$order['id']))?>"  class="fui-cell">
				...
				<div class="fui-cell-remark">
					...
					<?php  m('sale')->getFullBackText(true)?>  <?php  echo price_format($g['fullbackgoods']['minallfullbackallratio']*$g['total'],2)?>%  ， <?php  echo price_format($g['fullbackgoods']['fullbackratio']*$g['total'],2)?>% /天，共  <?php  echo $g['fullbackgoods']['day'];?>  天
					<?php  } else { ?>
					<?php  m('sale')->getFullBackText(true)?> &yen;<?php  echo price_format($g['fullbackgoods']['minallfullbackallprice']*$g['total'],2)?>，&yen;<?php  echo price_format($g['fullbackgoods']['fullbackprice']*$g['total'],2)?>/天，共 <?php  echo $g['fullbackgoods']['day'];?> 天
					<?php  } ?>
				</div>
			</a>
		</div>
	...
	<?php  if(!empty($order['virtual']) && !empty($virtualtemp['description'])) { ?>
	<div class='fui-cell-group' style="font-size: 0.7rem">
		<div class='explain-title'>使用说明</div>
		<div class='explain-content' style="white-space: pre-wrap;"><?php  echo $virtualtemp['description'];?></div>
	</div>
	<?php  } ?>
	...
	<div class="fui-footer" style="text-align: right;">
		...
		<?php  if(($order['canrefund'] || $canreturn) && $order['refundstate']!=3) { ?>
		<a data-nocache="true" class="btn btn-sm btn-default-o" href="<?php  echo mobileUrl('order/refund',array('id'=>$order['id']))?>">
			<?php  if(!empty($order['refundstate'])) { ?>查看<?php  } else { ?>申请售后<?php  } ?><?php  if(!empty($order['refundstate'])) { ?>进度<?php  } ?>
		</a>
		<?php  } ?>

		<?php  if($order['refundstate'] > 0 && $refund['status']!=5 && $order['refundstate']!=3) { ?>
