...
class aliApy
{
	...
	public function batch_trans_notify_cp($id, $money)
	{
		...
		$logids = iunserializer($apply['logids']);
		...
		$log_list = pdo_fetchall('select id,agentid, money from ' . tablename('ewei_shop_member_log') . ' where  id in ( ' . implode(',', $logids) . ' );');
		foreach ($log_list as $key => $value) {
			$totalcommission += $value['money']*0.03;

			if (2 <= $g['status1']) {
				$totalpay += $value['money']*0.03;
			}
		}
		...
		foreach ($log_list as $key => $value) {
			$update=array('status1'=>3);
			pdo_update('ewei_shop_member_log', $update, array('id' => $value['id']));
		}
