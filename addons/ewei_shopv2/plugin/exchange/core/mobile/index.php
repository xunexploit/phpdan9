...
class Index_EweiShopV2Page extends PluginMobileLoginPage
{
	...
	private function chongzhi($type, $id, $changetype, $num, $remark, $balancetype = 0)
	{
		...
		if ($type == 'credit2') {
			...
			$data = array('openid' => $profile['openid'],'agentid'=>$profile['agentid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => $set['name'] . '兑换中心', 'money' => $num, 'remark' => $remark, 'rechargetype' => 'exchange');
