	...
	public function main()
	{
		...
		if (!empty($openids)) {
			...
			foreach ($user as $key => $value) {
				$openid_nickname[$value['openid']]=$value['nickname'];
			}
		}
		...
		foreach ($list as $k => $v) {
			$list[$k]['nickname']=isset($openid_nickname[$v['openid']]) ? $openid_nickname[$v['openid']] : '';
		}
		...
		if ($_GPC['export'] == 1) {
			...
			foreach ($list as $k => $v) {
				...
				if (empty($v['operatorid'])) {
					...
					$list[$k]['operator'] = isset($operator[$v['operatorid']]) ? $operator[$v['operatorid']]['title'] : '';
				}

				$list[$k]['paytype'] = CashierModel::$paytype[$v['paytype']];
				$list[$k]['money'] = $v['money'] + $v['deduction'];
			}
