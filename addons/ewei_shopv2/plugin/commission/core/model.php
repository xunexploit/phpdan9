...
if (!class_exists('CommissionModel')) {
    class CommissionModel extends PluginModel {
        ...
        public function calculate($orderid = 0, $update = true, $order_agentid = NULL) {
            ...
            $order      = pdo_fetch('select agentid,price,goodsprice,deductcredit2,discountprice,isdiscountprice,dispatchprice,changeprice,ispackage,packageid,couponprice,buyagainprice,deductprice,deductenough,merchdeductenough,grprice from ' . tablename('ewei_shop_order') . ' where id=:id limit 1', array(
                ':id' => $orderid
            ));
          ...
          g.commission3_rate,g.commission3_pay,g.commission,og.commissions,og.seckill,og.seckill_taskid,og.seckill_timeid from ' . tablename('ewei_shop_order_goods') . '  og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id = og.goodsid ' . $condition;
                ...
                $goods = pdo_fetchall('select og.id,og.realprice,og.total,g.hasoption,og.goodsid,og.optionid,g.hascommission,g.nocommission, g.commission1_rate,g.commission1_pay,g.commission2_rate,g.commission2_pay,
                  g.commission3_rate,g.commission3_pay,g.commission,og.commissions,og.seckill,og.seckill_taskid,og.seckill_timeid from ' . tablename('ewei_shop_order_goods') . '  og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id = og.goodsid' . ' where og.orderid=:orderid and og.uniacid=:uniacid', array(
                    ':orderid' => $orderid,
                    ':uniacid' => $_W['uniacid']
                ));
            }
                    ...
                    if (!empty($seckill_goods)) {
                        ...
                        if (empty($cinfo['nocommission'])) {
                            ...
                            if ($cinfo['hascommission'] == 1) {
                                ...
                                $cinfo['commission2'] = array(
                                    'default' => 2 <= $set['level'] ? round($set['commission2'] * $price / 100, 2) . '' : 0
                                );
                                $cinfo['commission3'] = array(
        ...
        public function getOrderCommissions($orderid = 0, $ogid = 0) {
            global $_W;
            $set         = $this->getSet();
            $agentid     = pdo_fetchcolumn('select agentid from ' . tablename('ewei_shop_order') . ' where id=:id limit 1', array(
                ':id' => $orderid
            ));
            $goods       = pdo_fetch('select commission1,commission2,commission3 from ' . tablename('ewei_shop_order_goods') . ' where id=:id and orderid=:orderid and uniacid=:uniacid and nocommission=0 limit 1', array(
                ':id' => $ogid,
                ':orderid' => $orderid,
                ':uniacid' => $_W['uniacid']
            ));
        ...
        public function getInfo($openid, $options = NULL) {
            ...
            if (1 <= $level) {
                if (in_array('ordercount0', $options)) {
                    $level1_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.paytype<>1 and o.status>=0 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                ...
                if (in_array('ordercount', $options)) {
                    $level1_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.paytype<>1 and o.status>=1 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                ...
                if (in_array('ordercount3', $options)) {
                    $level1_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.paytype<>1 and o.status>=3 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                ...
                if (in_array('total', $options)) {
                    $level1_commissions = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.paytype<>1 and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));

                    $log_info = pdo_fetchall('select * from ' . tablename('ewei_shop_member_log') . ' where agentid=:agentid and uniacid=:uniacid and status=1 and type=0 and rechargetype<>"system" and rechargetype<>"system1"', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid']));

                    if(!empty($log_info)){
                        foreach ($log_info as $key => $value) {
                            $commission_total+=$value['money']*0.03;
                        }
                    }
                ...
                if (in_array('ok', $options)) {
                    $level1_commissions = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . (' where o.agentid=:agentid and o.paytype<>1 and o.status>=3 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ') and og.status1=0  and o.uniacid=:uniacid and o.isparent=0'), array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));

                    $log_info = pdo_fetchall('select * from ' . tablename('ewei_shop_member_log') . ' where agentid=:agentid and uniacid=:uniacid and status1=0 and status=1 and type=0 and rechargetype<>"system" and rechargetype<>"system1"', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid']));

                    if(!empty($log_info)){
                        foreach ($log_info as $key => $value) {
                            $commission_ok+=$value['money']*0.03;
                        }
                    }
                ...
                if (in_array('lock', $options)) {
                    $level1_commissions1 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . (' where o.agentid=:agentid and o.paytype<>1 and o.status>=3 and og.nocommission=0 and (' . $time . ' - o.finishtime <= ' . $day_times . ')  and og.status1=0  and o.uniacid=:uniacid and o.isparent=0'), array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                ...
                if (in_array('apply', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.paytype<>1 and o.status>=3 and og.status1=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));

                    $log_info = pdo_fetchall('select * from ' . tablename('ewei_shop_member_log') . ' where agentid=:agentid and uniacid=:uniacid and status1=1 and status=1 and type=0 and rechargetype<>"system" and rechargetype<>"system1"', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid']));

                    if(!empty($log_info)){
                        foreach ($log_info as $key => $value) {
                            $commission_apply+=$value['money']*0.03;
                        }
                    }
                ...
                if (in_array('check', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.paytype<>1 and o.status>=3 and og.status1=2 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));

                    $log_info = pdo_fetchall('select * from ' . tablename('ewei_shop_member_log') . ' where agentid=:agentid and uniacid=:uniacid and status1=2 and status=1 and type=0 and rechargetype<>"system" and rechargetype<>"system1"', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid']));

                    if(!empty($log_info)){
                        foreach ($log_info as $key => $value) {
                            $commission_check+=$value['money']*0.03;
                        }
                    }
                ...
                if (in_array('pay', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.paytype<>1 and o.status>=3 and og.status1=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));

                    $log_info = pdo_fetchall('select * from ' . tablename('ewei_shop_member_log') . ' where agentid=:agentid and uniacid=:uniacid and status1=3 and status=1 and type=0 and rechargetype<>"system" and rechargetype<>"system1"', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid']));

                    if(!empty($log_info)){
                        foreach ($log_info as $key => $value) {
                            $commission_pay+=$value['money']*0.03;
                        }
                    }
                ...
                if (in_array('wait', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.paytype<>1 and (o.status=2 or o.status=1) and og.status1=0  and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                        ':uniacid' => $_W['uniacid'],
                        ':agentid' => $member['id']
                    ));
                ...
                if (in_array('fail', $options)) {
                    ...
                    if(!empty($log_info)){
                        foreach ($log_info as $key => $value) {
                            $commission_fail+=$value['money']*0.03;
                        }
                    }
            ...
            if (2 <= $level) {
                if (0 < $level1) {
                    ...
                    if (in_array('ordercount', $options)) {
                        $level2_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.paytype<>1 and o.status>=1 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('ok', $options)) {
                        $level2_commissions = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . (')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and o.paytype<>1 and og.status2=0 and og.nocommission=0  and o.uniacid=:uniacid and o.isparent=0'), array(
                            ':uniacid' => $_W['uniacid']
                        ));
                        foreach ($level2_commissions as $c) {
                            $commissions = iunserializer($c['commissions']);
                    ...
                    if (in_array('pay', $options)) {
                        $level2_commissions3 = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and o.paytype<>1 and og.status2=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                        foreach ($level2_commissions3 as $c) {
                            $commissions = iunserializer($c['commissions']);
                    ...
                    if (in_array('wait', $options)) {
                        $level2_commissions3 = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and (o.status=1 or o.status=2) and o.paytype<>1 and og.status2=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                        foreach ($level2_commissions3 as $c) {
                            $commissions = iunserializer($c['commissions']);
            ...
            if (3 <= $level) {
                if (0 < $level2) {
                    ...
                    if (in_array('ordercount', $options)) {
                        $level3_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ') and o.paytype<>1 and o.status>=1 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('ok', $options)) {
                        $level3_commissions = pdo_fetchall('select og.commission3,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . (')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and og.status3=0 and o.paytype<>1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0'), array(
                            ':uniacid' => $_W['uniacid']
                        ));
                        foreach ($level3_commissions as $c) {
                            $commissions = iunserializer($c['commissions']);
                    ...
                    if (in_array('apply', $options)) {
                        $level3_commissions2 = pdo_fetchall('select og.commission3,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ') and o.paytype<>1 and o.status>=3 and og.status3=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('check', $options)) {
                        $level3_commissions3 = pdo_fetchall('select og.commission3,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ') and o.paytype<>1 and o.status>=3 and og.status3=2 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    ...
                    if (in_array('pay', $options)) {
                        $level3_commissions3 = pdo_fetchall('select og.commission3,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ') and o.paytype<>1 and o.status>=3 and og.status3=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                        foreach ($level3_commissions3 as $c) {
                            $commissions = iunserializer($c['commissions']);
                    ...
                    if (in_array('wait', $options)) {
                        $level3_commissions3 = pdo_fetchall('select og.commission3,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ') and o.paytype<>1 and ( o.status=1 or o.status=2) and og.status3=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(
                            ':uniacid' => $_W['uniacid']
                        ));
                        foreach ($level3_commissions3 as $c) {
                            $commissions = iunserializer($c['commissions']);
        ...
        public function getAgentsDownNum($openid = NULL) {
            ...
            $level1      = (int) pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where agentid=:agentid and uniacid=:uniacid limit 1', array(
                ':agentid' => $member['id'],
                ':uniacid' => $_W['uniacid']
            ));
            if (2 <= $set['level'] && 0 < $levelcount1) {
                $level2 = (int) pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ') and uniacid=:uniacid limit 1', array(
                    ':uniacid' => $_W['uniacid']
                ));
            }
            if (3 <= $set['level'] && 0 < $levelcount2) {
                $level3 = (int) pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ') and uniacid=:uniacid limit 1', array(
                    ':uniacid' => $_W['uniacid']
                ));
            }
        ...
        public function checkAgent($openid = '') {
            ...
            if ($open_redis) {
                ...
                if (!$first) {
                    $first = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid limit 1', array(
                        ':uniacid' => $_W['uniacid']
                    ));
                ...
                $first   = 0;
                $member1 = pdo_fetch('select id,openid from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid limit 1', array(
                    ':uniacid' => $_W['uniacid']
                ));
                if ($member1) {
                    $first   = 1;
                    $member2 = pdo_fetch('select id,openid from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid and id<>:fid limit 1', array(
                        ':uniacid' => $_W['uniacid'],
                        ':fid' => $member1['id']
                    ));
                    if ($member2) {
            ...
            if (empty($set['become'])) {
                ...
                $ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_order') . (' where uniacid=:uniacid and openid=:openid and status>=' . $order_status . ' limit 1'), array(
                    ':uniacid' => $_W['uniacid'],
                    ':openid' => $openid
                ));
                ...
                $moneycount = pdo_fetchcolumn('select sum(price) from ' . tablename('ewei_shop_order') . (' where uniacid=:uniacid and openid=:openid and status>=' . $order_status . ' limit 1'), array(
                    ':uniacid' => $_W['uniacid'],
                    ':openid' => $openid
                ));
                ...
                if ($set['become'] == 4) {
                    ...
                    $goods      = pdo_fetch('select id,title,thumb,marketprice from' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
                        ':id' => $set['become_goodsid'],
                        ':uniacid' => $_W['uniacid']
                    ));
                    $goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_order_goods') . ' og ' . '  left join ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . (' where og.goodsid=:goodsid and o.openid=:openid and o.status>=' . $order_status . ' and og.createtime >= ' . $time . ' limit 1'), array(
            ...
            if (!$isagent) {
                if (intval($set['become']) == 4 && !empty($set['become_goodsid'])) {
                    if (empty($set['become_order'])) {
                        $order_goods = pdo_fetchall('select goodsid from ' . tablename('ewei_shop_order_goods') . ' where orderid=:orderid and uniacid=:uniacid  ', array(
                            ':uniacid' => $_W['uniacid'],
                            ':orderid' => $order['id']
                        ), 'goodsid');
        ...
        public function getLevels($all = true, $default = false) {
            ...
            global $_S;
            if ($all) {
                $levels = pdo_fetchall('select * from ' . tablename('ewei_shop_commission_level') . ' where uniacid=:uniacid order by commission1 asc', array(
                    ':uniacid' => $_W['uniacid']
                ));
            } else {
                $levels = pdo_fetchall('select * from ' . tablename('ewei_shop_commission_level') . ' where uniacid=:uniacid and (ordermoney>0 or commissionmoney>0) order by commission1 asc', array(
                    ':uniacid' => $_W['uniacid']
                ));
            }
            if ($default) {
        ...
        public function upgradeLevelByOrder($openid) {
            ...
            if ($leveltype == 4 || $leveltype == 5) {
                ...
                $orders     = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.openid=:openid and o.status>=3 and o.uniacid=:uniacid limit 1', array(
                    ':uniacid' => $_W['uniacid'],
                    ':openid' => $openid
                ));
                ...
                if ($leveltype == 4) {
                    ...
                    if ($leveltype == 5) {
                        $newlevel = pdo_fetch('select * from ' . tablename('ewei_shop_commission_level') . (' where uniacid=:uniacid  and ' . $ordercount . ' >= ordercount and ordercount>0  order by ordercount desc limit 1'), array(
                            ':uniacid' => $_W['uniacid']
                        ));
                        if (empty($newlevel)) {
                            return NULL;
        ...
        public function upgradeLevelByAgent($openid) {
            ...
            if ($leveltype == 6 || $leveltype == 8) {
                ...
                foreach ($agents as $agent) {
                    ...
                    if ($leveltype == 6) {
                        $downs1 = pdo_fetchall('select id from ' . tablename('ewei_shop_member') . ' where agentid=:agentid and uniacid=:uniacid ', array(
                            ':agentid' => $m['id'],
                            ':uniacid' => $_W['uniacid']
                        ), 'id');
                        $downcount += count($downs1);
                        if (!empty($downs1)) {
                            $downs2 = pdo_fetchall('select id from ' . tablename('ewei_shop_member') . ' where agentid in( ' . implode(',', array_keys($downs1)) . ') and uniacid=:uniacid', array(
                                ':uniacid' => $_W['uniacid']
                            ), 'id');
                            $downcount += count($downs2);
                            if (!empty($downs2)) {
                                $downs3 = pdo_fetchall('select id from ' . tablename('ewei_shop_member') . ' where agentid in( ' . implode(',', array_keys($downs2)) . ') and uniacid=:uniacid', array(
                                    ':uniacid' => $_W['uniacid']
                                ), 'id');
                                $downcount += count($downs3);
                            }
                        }
                        $newlevel = pdo_fetch('select * from ' . tablename('ewei_shop_commission_level') . (' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0  order by downcount desc limit 1'), array(
                            ':uniacid' => $_W['uniacid']
                        ));
                    } else {
                        if ($leveltype == 8) {
                ...
                if ($leveltype == 7) {
                    $downcount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member') . ' where agentid=:agentid and uniacid=:uniacid ', array(
                        ':agentid' => $m['id'],
                        ':uniacid' => $_W['uniacid']
                    ));
                    $newlevel  = pdo_fetch('select * from ' . tablename('ewei_shop_commission_level') . (' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0  order by downcount desc limit 1'), array(
        ...
        public function upgradeLevelByGoods($openid, $orderid) {
            ...
            $orders = pdo_fetchall('select og.goodsid from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.openid=:openid and o.status>=3 and o.uniacid=:uniacid and og.orderid=:orderid', array(
                ':uniacid' => $_W['uniacid'],
                ':openid' => $openid,
                'orderid' => $orderid
            ));
            if ($leveltype == 11) {
                $newlevel = pdo_fetchall('select * from ' . tablename('ewei_shop_commission_level') . ' where uniacid=:uniacid  ', array(
                    ':uniacid' => $_W['uniacid']
                ), 'level');
...
感谢您的支持',
                    ...
                    $msg     = array(
                        'keyword1' => isset($tm['commission_agent_newtitle']) && trim($tm['commission_agent_newtitle']) == '新增下级通知' ? '' : array(
                            'value' => '新增下级通知',
                            'color' => '#73a68d'
                        ),
                        'keyword2' => array(
                            'value' => !empty($tm['commission_agent_newtitle']) ? $tm['commission_agent_newtitle'] : '恭喜您新增下级成员',
                            'color' => '#73a68d'
                        ),
                        'keyword3' => array(
                            'value' => $message,
...
';
                        $message  = array(
                            ...
                            'keyword2' => array(
                                'title' => '业务内容',
                                'value' => '您的下级' . $data['nickname'] . '已经成功付款',
                                'color' => '#000000'
                            ),
                            'keyword3' => array(
                                'title' => '处理结果',
...
感谢您的支持',
                        ...
                        $msg     = array(
                            ...
                            'keyword2' => array(
                                'value' => !empty($tm['commission_order_paytitle']) ? $tm['commission_order_paytitle'] : '您有下级付款了'
                            ),
                            'keyword3' => array(
                                'value' => $message
                            ),
...
';
                            $message  = array(
                                ...
                                'keyword2' => array(
                                    'title' => '业务内容',
                                    'value' => '您的下级' . $data['nickname'] . '已经确认收货',
                                    'color' => '#000000'
                                ),
                                'keyword3' => array(
                                    'title' => '处理结果',
...
感谢您的支持',
        ...
        public function getTotals() {
            global $_W;
            return array(
                'total1' => pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_commission_apply') . ' where status=1 and uniacid=:uniacid limit 1', array(
                    ':uniacid' => $_W['uniacid']
                )),
                'total2' => pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_commission_apply') . ' where status=2 and uniacid=:uniacid limit 1', array(
                    ':uniacid' => $_W['uniacid']
                )),
                'total3' => pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_commission_apply') . ' where status=3 and uniacid=:uniacid limit 1', array(
                    ':uniacid' => $_W['uniacid']
                )),
                'total_1' => pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_commission_apply') . ' where status=-1 and uniacid=:uniacid limit 1', array(
                    ':uniacid' => $_W['uniacid']
                ))
            );
        }
        protected function sendNotice($touser, $tm, $tag, $datas, $member, $msg) {
            ...
            if (!empty($tm['is_advanced']) && !empty($tm[$tag])) {
                $advanced_template = pdo_fetch('select * from ' . tablename('ewei_shop_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(
                    ':id' => $tm[$tag],
                    ':uniacid' => $_W['uniacid']
                ));
        ...
        public function getRepurchase($openid, array $time) {
            ...
            foreach ($time as $value) {
                ...
                if (!empty($repurchase_price)) {
                    $order_price         = (double) pdo_fetchcolumn('SELECT SUM(price) as price FROM ' . tablename('ewei_shop_order') . ' WHERE `uniacid`=:uniacid AND `openid`=:openid AND `status`>2 AND `createtime` BETWEEN :time1 AND :time2', array(
                        ':uniacid' => $_W['uniacid'],
                        ':openid' => $openid,
                        ':time1' => $time1,
                        ':time2' => $time2
                    ...
                    $residue_price       = (double) pdo_fetchcolumn('SELECT SUM(repurchase) FROM ' . tablename('ewei_shop_commission_repurchase') . ' WHERE `uniacid`=:uniacid AND `openid`=:openid AND `year`=:year AND `month`=:month', array(
                        ':uniacid' => $_W['uniacid'],
                        ':openid' => $openid,
                        ':year' => $year_month[0],
                        ':month' => $year_month[1]
        ...
        public function getAllDown($openid) {
            ...
            $uid = (int) $openid;
            if ($uid == 0) {
                $info = pdo_fetch('select id,openid,uniacid,agentid,isagent,status from ' . tablename('ewei_shop_member') . ' where  openid=:openid and uniacid=:uniacid limit 1', array(
                    ':uniacid' => $_W['uniacid'],
                    ':openid' => $openid
                ));
                if (empty($info)) {
            ...
            $agents  = pdo_fetchall('select id,openid,uniacid,agentid,isagent,nickname,agenttime,createtime,avatar,status,realname,mobile,weixin from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid and agentid=:agentid', array(
                ':uniacid' => $_W['uniacid'],
                ':agentid' => $uid
            ));
            $ids     = array();
        ...
        public function getAllDownOrder($openid, $start = 0, $end = 0) {
            ...
            if (!empty($down['openids'])) {
                $order = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_order') . ' WHERE uniacid=:uniacid AND openid IN (\'' . implode('\',\'', $down['openids']) . '\') AND createtime BETWEEN :time1 AND :time2 AND ccard>0', array(
                    ':uniacid' => $_W['uniacid'],
                    ':time1' => $start,
                    ':time2' => $end
                ));
        ...
        public function saveRelation($id, $pid, $level = 1, $later = array()) {
            ...
            if (!empty($later)) {
                if (self::inArray($pid, $later)) {
                    $member = pdo_fetch('select * from ' . tablename('ewei_shop_member') . ' where id = :id', array(
                        ':id' => $pid
                    ));
                    return $member;
                }
            ...
            $level   = max($level, 1);
            if ($level == 1 && 0 < $pid) {
                pdo_insert('ewei_shop_commission_relation', array(
                    'id' => $id,
                    'pid' => $pid,
                    'level' => $level
                ), true);
            }
            $childs = pdo_fetchall('select id,`level` from ' . tablename('ewei_shop_commission_relation') . ' where pid = :pid', array(
                ':pid' => $id
            ));
            if (!empty($childs)) {
                foreach ($childs as $child) {
                    if (0 < $pid) {
                        ...
                        pdo_query('delete from  `ims_ewei_shop_commission_relation` where id = :id and `level` > :level', array(
                            'id' => $child['id'],
                            ':level' => $child['level']
                        ));
                    }
            ...
            $parent = pdo_fetch('select id,uniacid,agentid from ' . tablename('ewei_shop_member') . ' where id = :id', array(
                ':id' => $pid
            ));
            if (!empty($parent)) {
        ...
        public function delRelation($id = 0) {
            $parent = pdo_fetchall('select * from ' . tablename('ewei_shop_commission_relation') . ' where id =:id', array(
                ':id' => $id
            ));
            if (!empty($parent)) {
                foreach ($parent as $p) {
            ...
            pdo_delete('ewei_shop_commission_relation', array(
                'id' => $id
            ));
        }
        /**
        ...
        public function modify($id, $pid = NULL) {
            ...
            $parent = pdo_fetchall('select * from ' . tablename('ewei_shop_commission_relation') . ' where pid =:pid', array(
                ':pid' => $id
            ));
