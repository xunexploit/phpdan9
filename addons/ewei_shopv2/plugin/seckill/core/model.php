...
class SeckillModel extends PluginModel
{
    ...
    public function getTaskGoods($taskid)
    {
        ...
        $redis_prefix = $this->get_prefix();
        $info = $redis->hGetAll((string) $redis_prefix . "goods_" . $taskid);
        if (empty($info)) {
            return false;
        }
        return $info;
    ...
    public function deleteTodaySeckill()
    {
        ...
        $month = date("m");
        $date = date("Y-m-d");
        redis()->hDel((string) $redis_prefix . "calendar_" . $year . "_" . $month, $date);
    }
    ...
    public function setSeckill($seckillinfo, $goods, $openid, $orderid, $status, $createtime)
    {
        ...
        $date = date("Y-m-d");
        $redis_prefix = $this->get_prefix();
        $key = (string) $redis_prefix . "queue_" . $date . "_" . $taskid . "_" . $timeid . "_" . $goodsid . "_" . $optionid;
        $redis = redis();
        if ($redis->ttl($key) == -1) {
    ...
    public function getSeckill($goodsid, $optionid = 0, $realprice = true, $openid = "")
    {
        ...
        if (!empty($timegoods)) {
            ...
            if ($timegoods[0]["end_time"] < $currenttime && !$roominfo["oldshow"]) {
                return false;
            }
    ...
    public function setOrderPay($orderid)
    {
        ...
        $redis_key = (string) $_W["uniacid"] . "_seckill_order_refund_" . $orderid;
        $redis = redis();
        ...
        foreach ($goods as $g) {
            $key = (string) $redis_prefix . "queue_" . $date . "_" . $g["seckill_taskid"] . "_" . $g["seckill_timeid"] . "_" . $g["goodsid"] . "_" . $g["optionid"];
            $queue = $redis->lGetRange($key, 0, -1);
    ...
    public function checkBuy($seckillinfo, $title, $unit = "")
    {
        ...
        if (0 < $seckillinfo["totalmaxbuy"] && $seckillinfo["totalmaxbuy"] <= $seckillinfo["selftotalcount"]) {
        ...
        if (0 < $seckillinfo["maxbuy"] && $seckillinfo["maxbuy"] <= $seckillinfo["selfcount"]) {
        ...
        if (!empty($seckillinfo["options"])) {
            ...
            $redis_key = (string) $_W["uniacid"] . "_seckill_stock_" . $seckillgoodsinfo["taskid"] . "_" . $seckillgoodsinfo["roomid"] . "_" . $seckillgoodsinfo["timeid"] . "_" . $seckillgoodsinfo["goodsid"] . "_" . $seckillgoodsinfo["optionid"] . "_" . $day;
            $redis = redis();
