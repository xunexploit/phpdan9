			...
			if (!empty($log) && $log['paystatus'] < 1 && $log['creditpay'] == 0) {
				...
				if ($status == 2) {
					if ($goods['goodstype'] == 1) {
						...
						$data = array('openid' => $log['openid'],'agentid'=>$member['agentid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => $set['name'] . '积分商城兑换余额', 'money' => $num, 'remark' => $creditstr, 'rechargetype' => 'creditshop');
