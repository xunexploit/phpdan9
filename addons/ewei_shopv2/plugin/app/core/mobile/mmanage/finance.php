		...
		if ($_W['ispost']) {
			...
			if ($type == 'credit1') {
				...
				if ($type == 'credit2') {
					...
					$data = array('openid' => $member['openid'],'agentid'=>$member['agentid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => $set['name'] . '会员充值', 'money' => $num, 'remark' => $remark, 'rechargetype' => 'system');
