...
class Pay_EweiShopV2Page extends AppMobilePage
{
	public function lottery()
	{
		...
		if (0 < $goods['money'] + $goods['dispatch'] && $log['paystatus'] < 1) {
			if ($log['paytype'] == 0) {
                m("member")->setCredit($openid, "credit2", 0 - ($goods["money"] + $goods["dispatch"]), "积分商城扣除余额度 " . ($goods["money"] + $goods["dispatch"]));
				$update['paystatus'] = 1;
			}
		...
		if ($status == 2 && $update['creditpay'] == 1) {
			if ($goods['goodstype'] == 1) {
				...
				$data = array('openid' => $openid,'agentid'=>$member['agentid'], 'logno' => $logno, 'uniacid' => $_W['uniacid'], 'type' => '0', 'createtime' => TIMESTAMP, 'status' => '1', 'title' => $set['name'] . '积分商城兑换余额', 'money' => $num, 'remark' => $creditstr, 'rechargetype' => 'creditshop');
