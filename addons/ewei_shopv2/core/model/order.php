...
class Order_EweiShopV2Model {
        ...
        foreach ($order_goods as $key => $value) {
            if (0 < $value['optionid']) {
                ...
                left join ' . tablename('ewei_shop_goods_option') . ' as go on go.goodsid = :id and go.id = ' . $value['optionid'] . '
                 where g.id=:id and g.uniacid=:uniacid limit 1', array(
                    ':id' => $value['goodsid'],
                    ':uniacid' => $uniacid
                ));
            } else {
                $goods = pdo_fetch('select * from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
                    ':id' => $value['goodsid'],
                    ':uniacid' => $uniacid
                ));
            }
            if (0 < $goods['isfullback']) {
                ...
                if (!empty($fullbackgoods) && $goods['hasoption'] && 0 < $value['optionid']) {
                    $option = pdo_fetch('select id,title,allfullbackprice,allfullbackratio,fullbackprice,fullbackratio,`day` from ' . tablename('ewei_shop_goods_option') . ' 
                        where id=:id and goodsid=:goodsid and uniacid=:uniacid and isfullback = 1 limit 1', array(
                        ':uniacid' => $uniacid,
    ...
    public function fullbackstop($orderid) {
        ...
        pdo_update('ewei_shop_fullback_log', array(
            'isfullback' => 1
        ), array(
            'id' => $fullback_log['id'],
            'uniacid' => $uniacid
    ...
    public function payResult($params) {
        ...
        $order        = pdo_fetch('select id,uniacid,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus,iscycelbuy from ' . tablename('ewei_shop_order') . ' where  ordersn=:ordersn and uniacid=:uniacid limit 1', array(
            ':uniacid' => $_W['uniacid'],
            ':ordersn' => $ordersn
        ));
        ...
        if (!empty($ispeerpay)) {
            $peerpay_info = (double) pdo_fetchcolumn('select SUM(price) price from ' . tablename('ewei_shop_order_peerpay_payinfo') . ' where pid=:pid limit 1', array(
                ':pid' => $ispeerpay['id']
            ));
            ...
            $order['status'] = 0;
            pdo_update('ewei_shop_order_peerpay', array(
                'status' => 1
            ), array(
                'id' => $ispeerpay['id']
            ));
    ...
    public function getChildOrder($orderid) {
        global $_W;
        $list = pdo_fetchall('select id,ordersn,status,finishtime,couponid,merchid  from ' . tablename('ewei_shop_order') . ' where  parentid=:parentid and uniacid=:uniacid', array(
            ':parentid' => $orderid,
            ':uniacid' => $_W['uniacid']
        ));
        return $list;
    ...
    public function afterPayResult($order, $ispeerpay = false) {
        if (p('task')) {
            ...
            $goodslist = pdo_fetchall('SELECT goodsid FROM ' . tablename('ewei_shop_order_goods') . ' WHERE orderid = :orderid AND uniacid = :uniacid', array(
                ':orderid' => $order['id'],
                ':uniacid' => $order['uniacid']
            ));
        ...
        if (p('lottery') && empty($ispeerpay)) {
            ...
            $res = p('lottery')->getLottery($order['openid'], 1, array(
                'money' => $order['price'],
                'paytype' => 1
            ));
            if ($order['isvirtualsend']) {
                $afterorder = p('lottery')->getLottery($order['openid'], 1, array(
                    'money' => $order['price'],
                    'paytype' => 2
                ));
                if ($afterorder) {
                    p('lottery')->getLotteryList($order['openid'], array(
                        'lottery_id' => $afterorder
                    ));
                }
            }
            if ($res) {
    ...
    public function setDeductCredit2($order) {
        ...
        if (0 < $order['deductcredit2']) {
            m('member')->setCredit($order['openid'], 'credit2', $order['deductcredit2'], array(
                '0',
                $_W['shopset']['shop']['name'] . ('购物返还抵扣余额 余额: ' . $order['deductcredit2'] . ' 订单号: ' . $order['ordersn'])
            ));
        }
    }
    /**
    ...
    public function setGiveBalance($orderid = '', $type = 0, $status = 0) {
        ...
        $goods   = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.money,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(
            ':uniacid' => $_W['uniacid'],
            ':orderid' => $orderid
        ));
        $balance = 0;
        ...
        if (0 < $balance) {
            ...
            if ($type == 1) {
                if ($order['status'] == 3) {
                    m('member')->setCredit($order['openid'], 'credit2', $balance, array(
                        0,
                        $shopset['name'] . '购物赠送余额 订单号: ' . $order['ordersn']
                    ));
                }
            } else {
                if( $type == 2 && $status == 3 && 1 <= $order["status"]) 
                {
                        m('member')->setCredit($order['openid'], 'credit2', 0 - $balance, array(
                            0,
                            $shopset['name'] . '购物取消订单扣除赠送余额 订单号: ' . $order['ordersn']
                        ));
    ...
    public function setStocksAndCredits($orderid = '', $type = 0, $flag = false) {
        global $_W;
        $tempflag = false;
        $order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status,isparent,paytype,isnewstore,storeid,istrade,status from ' . tablename('ewei_shop_order') . ' where id=:id limit 1', array(
            ':id' => $orderid
        ));
        ...
        foreach ($goods as $g) {
            if (0 < $newstoreid) {
                ...
                $goods_item      = pdo_fetch('select total as goodstotal from' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
                    ':id' => $g['goodsid'],
                    ':uniacid' => $_W['uniacid']
                ));
            ...
            if (!empty($stocktype)) {
                ...
                if (!empty($g['optionid'])) {
                    ...
                    if (!empty($option) && $option['stock'] != -1) {
                        ...
                        if ($stock != -1) {
                            if (0 < $newstoreid) {
                                pdo_update('ewei_shop_newstore_goods_option', array(
                                    'stock' => $stock
                                ), array(
                                    'uniacid' => $_W['uniacid'],
                                    'goodsid' => $g['goodsid'],
                                    'id' => $store_goods_option['id']
                                ));
                            } else {
                ...
                if ((!empty($g['goodstotal']) && $stocktype == -1 || $stocktype == 1) && $g['goodstotal'] != -1) {
                    ...
                    if ($totalstock != -1) {
                        if (0 < $newstoreid) {
                            pdo_update('ewei_shop_newstore_goods', array(
                                'stotal' => $totalstock
                            ), array(
                                'uniacid' => $_W['uniacid'],
                                'id' => $store_goods['id']
                ...
                if ($type == 1) {
                    if (1 <= $order['status']) {
                        ...
                        if (0 < $credits && $table_flag) {
                            ...
                            $send_record = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_order_buysend') . ' WHERE orderid = :orderid AND uniacid = :uniacid AND openid = :openid', array(
                                ':orderid' => $orderid,
                                ':uniacid' => $_W['uniacid'],
                                ':openid' => $order['openid']
                            ));
                            if ($send_record) {
                                pdo_update('ewei_shop_order_buysend', $send_data, array(
                                    'id' => $send_record['id']
                                ));
                            } else {
        ...
        if ($table_flag) {
            ...
            if (0 < $credits) {
                ...
                if ($type == 3) {
                    ...
                    if ($type == 2) {
                        if ($order['status'] == 3) {
                            m('member')->setCredit($order['openid'], 'credit1', 0 - $credits, array(
                                0,
    ...
    public function getGoodsDiscountPrice($g, $level, $type = 0, $memberCardId = 0) {
        ...
        if (!empty($level['id'])) {
            $level = pdo_fetch('select * from ' . tablename('ewei_shop_member_level') . ' where id=:id and uniacid=:uniacid and enabled=1 limit 1', array(
                ':id' => $level['id'],
                ':uniacid' => $_W['uniacid']
            ));
    ...
    public function validate_city_express($address) {
        ...
        $city_express      = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_city_express') . ' WHERE uniacid=:uniacid and merchid=0 limit 1', array(
            ':uniacid' => $_W['uniacid']
        ));
        if (!empty($city_express['enabled'])) {
            ...
            if (!empty($address)) {
                ...
                if ($data['status'] == 1 && 0 < $data['count']) {
                    ...
                    pdo_update('ewei_shop_member_address', $addres, array(
                        'id' => $addres['id'],
                        'uniacid' => $_W['uniacid']
                    ));
    ...
    public function changeParentOrderPrice($parent_order) {
        global $_W;
        $id   = $parent_order['id'];
        $item = pdo_fetch('SELECT price,ordersn2,dispatchprice,changedispatchprice FROM ' . tablename('ewei_shop_order') . ' WHERE id = :id and uniacid=:uniacid', array(
            ':id' => $id,
            ':uniacid' => $_W['uniacid']
        ));
        if (!empty($item)) {
    ...
    public function getOrderCommission($orderid, $agentid = 0) {
        global $_W;
        if (empty($agentid)) {
            $item = pdo_fetch('select agentid from ' . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid Limit 1', array(
                'id' => $orderid,
                ':uniacid' => $_W['uniacid']
            ));
            if (!empty($item)) {
    ...
    public function checkOrderGoods($orderid) {
        ...
        $item      = pdo_fetch('select * from ' . tablename('ewei_shop_order') . '  where  id = :id and uniacid=:uniacid limit 1', array(
            ':id' => $orderid,
            ':uniacid' => $uniacid
        ));
    ...
    public function getVerifyCardNumByOrderid($orderid) {
        ...
        $num = pdo_fetchcolumn('select SUM(og.total)  from ' . tablename('ewei_shop_order_goods') . ' og
		 ...
		 where og.uniacid=:uniacid  and og.orderid =:orderid and g.cardid>0', array(
            ':uniacid' => $_W['uniacid'],
            ':orderid' => $orderid
        ));
        return $num;
    ...
    public function checkisonlyverifygoods($orderid) {
        ...
        $num = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('ewei_shop_order_goods') . ' og
		 inner join ' . tablename('ewei_shop_goods') . ' g on og.goodsid = g.id
		 where og.uniacid=:uniacid  and og.orderid =:orderid and g.type<>5', array(
            ':uniacid' => $_W['uniacid'],
            ':orderid' => $orderid
        ));
        $num = intval($num);
    ...
    public function checkhaveverifygoodlog($orderid) {
        ...
        $num = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('ewei_shop_verifygoods_log') . ' vl
		 ...
		 where v.uniacid=:uniacid  and v.orderid =:orderid ', array(
            ':uniacid' => $_W['uniacid'],
            ':orderid' => $orderid
        ));
        $num = intval($num);
    ...
    public function getOrderVirtual($order = array()) {
        ...
        $ordervirtual = array();
        $virtual_type = pdo_fetch('select fields from ' . tablename('ewei_shop_virtual_type') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1 ', array(
            ':id' => $order['virtual'],
            ':uniacid' => $_W['uniacid'],
    ...
    public function getdadacity() {
        ...
        $resp = ihttp_request($url, json_encode($reqParams), array(
    ...
    public function dada_send($order) {
        ...
        $cityexpress = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_city_express') . ' WHERE uniacid=:uniacid AND merchid=:merchid', array(
            ':uniacid' => $_W['uniacid'],
            ':merchid' => 0
        ));
