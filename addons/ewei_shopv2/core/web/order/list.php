                                    ...
                                    if ($searchfield == "saler") {
                                        ...
                                        if ($searchfield == "verifycode") {
                                            ...
                                            if ($searchfield == "store") {
                                                if (p("merch")) {
                                                    $sqlcondition = " left join " . tablename("ewei_shop_store") . " store on ( store.id = vgl.storeid or store.id = vol.storeid ) and store.uniacid=o.uniacid and o.ismerch=0\r\n                    left join " . tablename("ewei_shop_merch_store") . " merstore on (  merstore.id = o.verifystoreid or merstore.id = vgl.storeid or merstore.id = vol.storeid )and merstore.uniacid=o.uniacid and o.ismerch=1";
                                                    $condition .= " AND (locate(:keyword,store.storename)>0 or locate(:keyword,merstore.storename)>0)";
                                                } else {
                                                    $sqlcondition = " left join " . tablename("ewei_shop_store") . " store on ( store.id = vgl.storeid or store.id = vol.storeid ) and store.uniacid=o.uniacid ";
        ...
        if (!empty($list)) {
            ...
            foreach ($list as &$value) {
                ...
                foreach ($order_goods as &$og) {
                    ...
                    if (!empty($level) && empty($agentid) && empty($og["nocommission"]) && $pt<>'1') {
