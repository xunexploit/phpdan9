...
class PosterProcessor extends PluginProcessor
{
    ...
    private function responseText($obj)
    {
        ...
        if (is_file(EWEI_SHOPV2_CORE . 'queue.pid')) {
            ...
            queue_push(new core\job\sendPoster(array('_W' => array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'siteroot' => $_W['siteroot'], 'sitescheme' => $_W['sitescheme'], 'siteurl' => $_W['siteurl'], 'attachurl' => $_W['attachurl'], 'ispost' => $_W['ispost'], 'isajax' => $_W['isajax'], 'setting' => $_W['setting'], 'config' => $_W['config']), 'openid' => $obj->message['from'], 'content' => $obj->message['content'], 'timestamp' => TIMESTAMP)));
        } else {
            $timeout = 4;
