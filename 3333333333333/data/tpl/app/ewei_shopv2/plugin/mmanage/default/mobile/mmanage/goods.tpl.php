...
<div class='fui-page fui-page-current'>
    ...
    <div class="head-menu">
        <!--<a><i class="icon icon-cascades"></i> 分类管理</a>-->
        <a class="batch-btn"><i class="icon icon-squarecheck"></i> 批量管理</a>
        <?php if(cv('goods.add')) { ?>
            <a class="external" href="<?php  echo mobileUrl('mmanage/goods/add')?>"><i class="icon icon-add3"></i> 添加商品</a>
        <?php  } ?>
    </div>
    ...
    <?php (!empty($this) && $this instanceof WeModuleSite) ? (include $this->template('mmanage/_tpl/goods', TEMPLATE_INCLUDEPATH)) : (include template('mmanage/_tpl/goods', TEMPLATE_INCLUDEPATH));?>

    <script type="text/javascript" src="../addons/ewei_shopv2/plugin/mmanage/static/js/init.js?v=<?php  echo time()?>"></script>
    <script language="javascript">
        require(['../addons/ewei_shopv2/plugin/mmanage/static/js/goods.js'],function(modal){
            modal.initList();
        });
    </script>
</div>
