...
<div class="js-system-module-detail" ng-controller="detailCtrl" ng-cloak>
	...
	<div class="user-head-info ">
		...
		<a ng-if="isFounder == 1 && moduleinfo.cloud_mid != ''" ng-href="{{ '' }}" target="_blank" class="btn btn-primary ">查看商城详情</a>
		<?php  } ?>
	</div>
	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && (upgradeInfo.from == 'local' || !upgradeInfo)">
		...
		<tr>
			<td>{{ branchname }}</td>
			<td><span class="label label-success">当前分支</span></td>
			<td>{{ moduleinfo.version }}</td>
			<td>线上：{{ upgradeInfo ? upgradeInfo.best_version : '暂无最新版本' }}<br><span ng-if="localUpgradeInfo">本地：{{localUpgradeInfo.best_version}}</span></td>
			<td class="text-right">
				<?php  if(permission_check_account_user('see_module_manage_system_ugrade')) { ?>
				...
				<a ng-if="upgradeInfo" href="<?php  echo url('module/manage-system/upgrade')?>module_name={{ moduleinfo.name }}" class="btn btn-primary">升级</a>
	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && upgradeInfo.from == 'cloud' && branch.displayorder >= upgradeInfo.site_branch.displayorder" ng-repeat="branch in upgradeInfo.branches">
		...
		<tr>
			...
			<td colspan="4" class="text-right">
				...
				<a href="" ng-if="branch.displayorder > upgradeInfo.site_branch.displayorder || (branch.displayorder == upgradeInfo.site_branch.displayorder && branch.id > upgradeInfo.site_branch.id)" class="color-default view-detail" target="_blank">查看分支详情</a>
			</td>
		</tr>
