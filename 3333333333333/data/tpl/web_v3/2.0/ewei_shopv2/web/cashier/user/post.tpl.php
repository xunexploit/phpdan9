...
<div class="page-header">
    当前位置：<span class="text-primary"><?php  if(!empty($item['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>收银台 <small><?php  if(!empty($item['id'])) { ?>修改【<?php  echo $item['title'];?>】<?php  } ?></small></span>
</div>
...
<div class="page-content">
    ...
    <div class="tabs-container">
        ...
        <div class="tab-content ">
            ...
            <div class="tab-pane <?php  if($_GPC['tab']=='notice') { ?>active<?php  } ?>" id="tab_notice"><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('cashier/user/post/notice', TEMPLATE_INCLUDEPATH)) : (include template('cashier/user/post/notice', TEMPLATE_INCLUDEPATH));?></div>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-9 col-xs-12">
            ...
            <a class="btn btn-default" href="<?php  echo webUrl('cashier/user')?>">返回列表</a>
        </div>
    </div>
    <?php  } ?>
    </form>
