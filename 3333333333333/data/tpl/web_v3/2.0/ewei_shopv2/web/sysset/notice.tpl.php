    ...
    <form action="" method="post" class="form-horizontal  form-validate" enctype="multipart/form-data" >
        ...
        <table class="table table-responsive">
            ...
            <tr>
                <td>审核成功通知</td>
                <td>
                    <select class="select2 is_advanced" name="data[march_type_success_template]">
                        <option value=''>[默认]审核成功通知</option>
                        <?php  if(is_array($template_group['march_type_success'])) { foreach($template_group['march_type_success'] as $template_val) { ?>
                        <option value="<?php  echo $template_val['id'];?>" <?php  if($data['march_type_success_template'] == $template_val['id'] ) { ?>selected<?php  } ?>><?php  echo $template_val['title'];?></option>
                        <?php  } } ?>
                    </select>
                </td>
                <td style="text-align: right;" class="is_advanced">
                    <label class="notice-default">
                        <input type="hidden" name="data[march_success]" value="<?php  echo intval($data['march_success'])?>" />
                        <input class="js-switch small checkone" data-type="tpl-advanced"  data-tag="march_type_success" type="checkbox" value="<?php  echo intval($data['march_success'])?>" <?php  if(empty($data['march_success'])) { ?>checked<?php  } ?>/>
                    </label>
                    <label style="display: none;">
                        <img src="../addons/ewei_shopv2/static/images/loading.gif" width="20" alt=""  onerror="this.src='../addons/ewei_shopv2/static/images/nopic.png'" />
                    </label>
            ...
            <tr>
                ...
                <td>
                    <select class="select2 is_advanced" name="data[recharge_ok_template]">
                        <option value=''>[默认]审核失败通知</option>
                        <?php  if(is_array($template_group['march_type_fail'])) { foreach($template_group['march_type_fail'] as $template_val) { ?>
                        <option value="<?php  echo $template_val['id'];?>" <?php  if($data['march_type_fail_template'] == $template_val['id'] ) { ?>selected<?php  } ?>><?php  echo $template_val['title'];?></option>
                        <?php  } } ?>
                    </select>
                </td>
                <td style="text-align: right;" class="is_advanced">
                    <label class="notice-default">
                        <input type="hidden" name="data[march_type]" value="<?php  echo intval($data['march_type'])?>" />
                        <input class="js-switch small checkone" data-type="tpl-advanced"  data-tag="march_type_fail" type="checkbox" value="<?php  echo intval($data['march_type'])?>" <?php  if(empty($data['march_type'])) { ?>checked<?php  } ?>/>
                    </label>
                    <label style="display: none;">
                        <img src="../addons/ewei_shopv2/static/images/loading.gif" width="20" alt=""  onerror="this.src='../addons/ewei_shopv2/static/images/nopic.png'" />
                    </label>
