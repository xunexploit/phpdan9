			...
			<div class="tab-content ">
				...
				<div class="tab-pane" id="sut_diyxcx" style="min-height: 100px;">
					<div class="row" style="margin: 50px 80px 50px;">
						...
						<div class="help-block text-danger">注意：必须是同一个公众号下的小程序appid，否则无法跳转,如果此处新添appid或更换appid，则需要重新发布审核</div>
					</div>
					<div class="row" style="margin: 50px 80px 50px;">
						<div class="input-group">
				...
				<div class="tab-pane" id="sut_quick">
					...
					<?php  if(is_array($quickList)) { foreach($quickList as $item) { ?>
					<div class="line">
						<nav title="选择" class="btn btn-default btn-sm" data-href="<?php  echo mobileUrl('quick',array('id'=>$item['id']),$full)?>">选择</nav>
				...
				<div class="tab-pane" id="sut_friendcoupon">
					<?php  if(!empty($activities)) { ?>
						<?php  if($platform=='wxapp' || $platform =='wxapp_tabbar') { ?>
							...
							<div class="line">
								<nav title="选择" class="btn btn-default btn-sm" data-href="/friendcoupon/index?id=<?php  echo $item['id'];?>">选择</nav>
								<div class="text"><?php  echo $item['title'];?></div>
							</div>
							<?php  } } ?>
						<?php  } else { ?>
							<?php  if(is_array($activities)) { foreach($activities as $item) { ?>
								<div class="line">
									<nav title="选择" class="btn btn-default btn-sm" data-href="<?php  echo mobileUrl('friendcoupon',array('id'=>$item['id']),$full)?>">选择</nav>
									<div class="text"><?php  echo $item['title'];?></div>
								</div>
							<?php  } } ?>
						<?php  } ?>
					<?php  } else { ?>
						<div class="tip">抱歉！未查询到好友瓜分券活动。</div>
					<?php  } ?>
				</div>
				<?php  } ?>
